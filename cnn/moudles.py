...
def data_load():
    ...
    for i in range(1, 6):
        ...
        label_train = torch.cat([label_train, torch.tensor(data_batch[b'labels'], dtype=torch.long, device=device)],
                                dim=0)
    ...
    return data_train, label_train, data_validation, label_validation
...
class AlexNet(nn.Module):
    def __init__(self, dpo):
        super(AlexNet, self).__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(3, 6, 3, 1, 1),  # 3 * 32 * 32   in_channels, out_channels, kernel_size
            nn.ReLU(),
            nn.MaxPool2d(2, 2),  # kernel_size, stride
            nn.Conv2d(6, 16, 3, 1, 1),  # 6 * 16 * 16
            nn.ReLU(),
            nn.MaxPool2d(2, 2),
            nn.Conv2d(16, 32, 3, 1, 1),  # 16 * 8 * 8
            nn.ReLU(),
            nn.MaxPool2d(2, 2),
            nn.Conv2d(32, 64, 3, 1, 1),  # 32 * 4 * 4
            nn.ReLU(),
            nn.MaxPool2d(2, 2)
        )
...
def train(net, data_iter, batch_size, optimizer, device, num_epochs, data_train, label_train,
    ...
    start = time.time()
    accuracy = numpy.zeros((2, num_epochs))
    for epoch in range(num_epochs):
        ...
        accuracy[0, epoch] = train_acc_sum / n
        accuracy[1, epoch] = test_acc

        if epoch % 50 == 49:
            print('epoch %4d, loss %.4f, train acc %.3f, test acc %.3f, time %.1f sec'
                  % (epoch + 1, train_l_sum / batch_count, train_acc_sum / n, test_acc, time.time() - start))
            start = time.time()

    return list([accuracy[0, :], accuracy[1, :]])


def train_net(dpo):
    timer = time.time()

    data_train, label_train, data_validation, label_validation = data_load()
    net = AlexNet(dpo=dpo)
    print(net)
    lr, num_epochs = 0.001, 1000
    batch_size = 512
    optimizer = torch.optim.Adam(net.parameters(), lr=lr)
    acc = train(net, data_iter, batch_size, optimizer, device, num_epochs, data_train, label_train,
          data_validation, label_validation)
